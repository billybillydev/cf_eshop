...
export const useSendContactEmail = async (name: string, email: string, message: string) => {
    try {
        ...
        const res = await sendContactEmailUseCase.execute(name, new EmailObject(email), message);
        return res;
    } catch (error) {
        console.log(error);
