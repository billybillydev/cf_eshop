...
export class ProductRepository extends JwtRepository implements ProductRepositoryInterface {
  ...
  async showAll(): Promise<ProductItemEntity[]> {
    try {
      ...
      return res.map(ProductItemEntity.transformToEntity);
    } catch (error) {
      console.error(error);
      return [];
    }
  ...
  async getById(id: IdObject): Promise<ProductEntity | null> {
    try {
      ...
      return res ? ProductEntity.transformToEntity(res) : null;
    } catch (error) {
      console.error(error);
      return null;
    }
  ...
  async filter(params: ProductFilterParameters): Promise<ProductItemEntity[]> {
    ...
    return products.map(ProductItemEntity.transformToEntity);
  }
  ...
  async paginate(
    ...
    return [
      products.map(ProductItemEntity.transformToEntity),
      total,
    ];
  }
}
